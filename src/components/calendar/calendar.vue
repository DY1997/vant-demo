...
<template>
    <div class="calendar">
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span v-for="y in years" :class="{'active':y==year}">{{y}}</span>
        </div>
...
//                return window.navigator.language.toLowerCase() == "zh-cn"?['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']:['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
                return ['01月', '02月', '03月', '04月', '05月', '06月', '07月', '08月', '09月', '10月', '11月', '11月']
            }
        },
        // 自定义事件
        events:  {
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (day == 0) {
                    ...
                    for (let j = 0; j < firstDayOfMonth; j++) {
                        ...
                        temp[line].push(Object.assign(
                            {day: k,disabled: true, month: (this.month - 1) < 0 ? 11 : (this.month - 1), year: (this.month - 1) < 0 ? (this.year - 1) : this.year}
                        ))
                        k++;
                    }
                }
                ...
                if (this.range) { // 范围
                    ...
                    if (parseInt(seletSplit[0]) == this.year && parseInt(seletSplit[1]) - 1 == this.month && parseInt(seletSplit[2]) == i) {
                     ...
                     // 没有默认值的时候显示选中今天日期
                        ...
                        temp[line].push(Object.assign(
                            {day: i,selected: true, month: this.month, year: this.year}
                        ))
                        this.today = [line, temp[line].length - 1]
                    }else{
                        // 普通日期
                        ...
                        let options = Object.assign(
                            {day: i,selected:false, month: this.month, year: this.year}
                        )
                        if (this.begin.length>0) {
                            let beginTime = Number(new Date(parseInt(this.begin[0]),parseInt(this.begin[1]) - 1,parseInt(this.begin[2])))
                            if (beginTime > Number(new Date(this.year, this.month, i))) options.disabled = true
                ...
                if (day == 6 && i < lastDateOfMonth) {
                    ...
                    for (let d=day; d < 6; d++) {
                        ...
                        temp[line].push(Object.assign(
                            {day: k,disabled: true, month: (this.month + 1) > 11 ? 0 : (this.month + 1), year: (this.month + 1) > 11 ? (this.year + 1) : this.year}
                        ))
                        k++
                    }
                    // 下个月除了补充的前几天开始的日期
...
//                            {day: d,disabled: true, month: this.month, year: this.year}
//                        ))
                    }
                }
            }
          ...
          // 当月剩余天数
          let currentMrestdays = daycount - new Date().getDate()
          // 下个月可选剩余天数
          let restdays = 30 - currentMrestdays
          this.days.forEach(v => {
            v.forEach(vv => {
              if (vv.month === currentMonth && vv.year === currentYear && vv.day < currentDay) {
                // 把当月已经过去的日期变为不可选择
                vv.disabled = true
              }
              if ((vv.month < currentMonth && vv.year === currentYear) || vv.year < currentYear) {
                // 把上一个月和去年的日期变为不可选择
                vv.disabled = true
              }
              // 同一年, 月份大于当月的
              if (vv.year === currentYear && vv.month > currentMonth) {
                vv.disabled = true
              }
              // 下一个月可选的日期
              if (vv.year === currentYear && vv.month === currentMonth + 1 && vv.day < restdays) {
                vv.disabled = false
              }
              // 跨年
              if (vv.year > currentYear) {
                vv.disabled = true
              }
              // 如果当月是12月, 下年一月份可以选择的日期
              if (vv.year === currentYear + 1 && vv.month === 0 && currentMonth === 11 && vv.day < restdays) {
                vv.disabled = false
              }
            })
          })
