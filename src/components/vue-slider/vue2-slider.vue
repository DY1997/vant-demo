<template>
  <div id="sliderA" ref="wrap" :class="['vue-slider-component', flowDirection, disabledClass, { 'vue-slider-has-label': piecewiseLabel }]" v-show="show" :style="wrapStyles" @click="wrapClick">
    <div ref="elem" aria-hidden="true" class="vue-slider" :style="[elemStyles, bgStyle]">
      ...
      <ul class="vue-slider-piecewise" id="timelis" ref="timelis">
        <!--:style="[piecewiseDotStyle, piecewiseObj.style]" -->
        <li v-for="(piecewiseObj, index) in piecewiseDotWrap" class="vue-slider-piecewise-item" :class="{'availabletime': piecewiseObj.status === 0, 'unavailabletime': piecewiseObj.status === 1}" :style="'left:' + index * 50 + 'px'" style="width: 50px" :key="index">
          <slot
            name="piecewise"
    ...
    piecewiseDotWrap () {
      ...
      for (let i = 0; i <= this.total; i++) {
        ...
        arr.push({
          ...
          status: this.timeObject[i] ? this.timeObject[i].status : 0
        })
      }
      return arr
    }
...
//      console.log((this.reverse ? (this.size - (e.clientX - this.offset) + this.$refs.wrap.offsetParent.scrollLeft) : (e.clientX - this.offset) + this.$refs.wrap.offsetParent.scrollLeft))
      return this.direction === 'vertical' ? (this.reverse ? (e.pageY - this.offset) : (this.size - (e.pageY - this.offset))) : (this.reverse ? (this.size - (e.clientX - this.offset) + this.$refs.wrap.offsetParent.scrollLeft) : (e.clientX - this.offset) + this.$refs.wrap.offsetParent.scrollLeft)
    },
    wrapClick (e) {
...
//      console.log('screenW', screenW)
      if (this.isRange) {
//        console.log(this.slider[0].style.transform)
  ...
  updated () {
    if (this.timeObject.length > 0) {
      for (var i = 0; i < this.timeObject.length; i++) {
        this.piecewiseDotWrap[i].status = this.timeObject[i].status
      }
      this.interval = this.interval + 1 - 1
    }
//    console.log('this.timeObject:', this.timeObject)
//    console.log('this.piecewiseDotWrap:', this.piecewiseDotWrap)
  },
  mounted () {
//    console.log('this.timeObject:', this.timeObject)
//    setTimeout(function () {
...
//    let timeLis = document.getElementById('.timelis')
//    console.log('timeLis:', this.$refs.timeLis)
...
<style>
  ...
  .vue-slider-component .availabletime {
     background: #5ECFEF;
  }
  .vue-slider-component .availabletime:last-child {
    width: 0px !important;
  }
  .vue-slider-component .unavailabletime {
    background: #9E9EA8;
  }
  .vue-slider-component .unavailabletime:last-child {
    width: 0px !important;
  }
  .vue-slider-component .unavailabletime:last-child .vue-slider-piecewise-dot ::after {
    display: block;
    content: '';
    width: 15px;
    height: 15px;
